...
template <typename Stamp> double to_sec(const Stamp &stamp) {
...
static RawPoints::Ptr
extract_raw_points(const PointCloud2 &points_msg,
  ...
  if (time_offset >= 0) {
    ...
    for (int i = 0; i < num_points; i++) {
      ...
      case PointField::FLOAT64:
        ...
        break;
      default:
        std::cerr << "warning: unsupported time type " << time_type
                  << std::endl;
        return nullptr;
      }
    }
...
// static RawPoints::Ptr
// extract_raw_points(const PointCloud2ConstPtr &points_msg,
//                    const std::string &intensity_channel = "intensity") {
//   (void)intensity_channel;
